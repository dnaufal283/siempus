...
<head>
    ...
    <title>Master Poli - SIEMPUS IT</title>
    <!-- Bootstrap 5 -->
...
<body>
    ...
    <div class="d-flex">
        ...
        <div class="sidebar shadow-sm">
            ...
            <nav class="nav flex-column">
                ...
                <a class="nav-link" href="/dashboard-it/users"><i class="fas fa-users-cog me-2 w-20px text-center"></i>
                    Manajemen User</a>
                <a class="nav-link active" href="/dashboard-it/poli"><i
                        class="fas fa-hospital me-2 w-20px text-center"></i>
                    Data Master Poli</a>
        ...
        <div class="content">
            ...
            <!-- Area Konten Master Poli -->
            <div class="p-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h5 class="fw-bold text-dark mb-0"><i class="fas fa-hospital me-2 text-secondary"></i> Data Master
                        Poli</h5>
                    <button class="btn btn-sm rounded-pill px-3 shadow-sm text-white"
                        ...
                        data-bs-target="#modalTambahPoli">
                    ...
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    @forelse($polis as $index => $p)
                                        <tr>
                                            <td class="ps-4">{{ $index + 1 }}</td>
                                            <td class="fw-bold text-dark">{{ $p->nama_poli }}</td>
                                            <td class="text-muted">{{ $p->deskripsi ?? '-' }}</td>
                                            <td class="text-center">
                                                <button class="btn btn-sm btn-outline-warning rounded-pill px-3"
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#modalEditPoli{{ $p->id }}">
                                                    <i class="fas fa-edit"></i> Edit
                                                </button>
                                                <form action="{{ route('it.poli.destroy', $p->id) }}" method="POST"
                                                    class="d-inline"
                                                    onsubmit="return confirm('Yakin ingin menghapus Poli {{ $p->nama_poli }}?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                        ...
                                        <!-- Modal Edit Poli -->
                                        <div class="modal fade" id="modalEditPoli{{ $p->id }}" tabindex="-1"
                                            aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content border-0 shadow">
                                                    <div class="modal-header text-white bg-warning">
                                                        <h5 class="modal-title text-dark fw-bold"><i
                                                                class="fas fa-edit me-2"></i>Edit Data Poli</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                            aria-label="Close"></button>
                                                    </div>
                                                    <form action="{{ route('it.poli.update', $p->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                        <div class="modal-body text-start">
                                                            <div class="mb-3">
                                                                ...
                                                                <input type="text" name="nama_poli"
                                                                    class="form-control" value="{{ $p->nama_poli }}"
                                                                    required>
                                                            </div>
                                                            <div class="mb-3">
                                                                <label class="form-label fw-medium text-dark">Deskripsi
                                                                    Keterangan</label>
                                                        ...
                                                        <div class="modal-footer bg-light">
                                                            ...
                                                            <button type="submit"
                                                                class="btn btn-warning fw-bold">Update Poli</button>
                                                        </div>
                                                    </form>
                                                </div>
                                            </div>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center py-4 text-muted">Belum ada data
                                                Master Poli yang ditambahkan.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                ...
                <div class="modal-dialog">
                    <div class="modal-content border-0 shadow">
                        <div class="modal-header text-white" style="background-color: var(--siempus-teal);">
                            <h5 class="modal-title"><i class="fas fa-plus-circle me-2"></i>Tambah Poli Baru</h5>
                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                                aria-label="Close"></button>
                        </div>
                        <form action="{{ route('it.poli.store') }}" method="POST">
                            @csrf
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label class="form-label fw-medium">Nama Poli</label>
                                    <input type="text" name="nama_poli" class="form-control" required
                                        placeholder="Contoh: Poli Umum">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label fw-medium">Deskripsi Keterangan</label>
                                    <textarea name="deskripsi" class="form-control" rows="3"
                                        placeholder="Contoh: Melayani pemeriksaan kesehatan umum awal..."></textarea>
                                </div>
                            </div>
                            <div class="modal-footer bg-light">
                                <button type="button" class="btn btn-secondary"
                                ...
                                <button type="submit" class="btn text-white"
                                    style="background-color: var(--siempus-teal);">Simpan Poli</button>
                            </div>
                        </form>
                    </div>
                </div>
